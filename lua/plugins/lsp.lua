    ...
    config = function()
        ...
        vim.api.nvim_create_autocmd("FileType", {
            ...
            callback = function()
                ...
                local root_dir
                if ok_util and util and type(util.root_pattern) == "function" then
                    root_dir = util.root_pattern(".git")(vim.fn.expand("%:p")) or vim.fn.getcwd()
                else
                ...
                if srv == "jdtls" then
                    ...
                    if ok_j and jdtls and type(jdtls.start_or_attach) == "function" then
                        jdtls.start_or_attach(opts)
                    else
                        vim.notify("jdtls non disponibile (require 'jdtls' fallito).", vim.log.levels.WARN)
                    end
                    return
                end

                if type(vim.lsp) == "table" and type(vim.lsp.config) == "function" and type(vim.lsp.enable) == "function" then
                    pcall(function()
                        vim.lsp.config(srv, opts)
                        vim.lsp.enable({ server = srv })
                    end)
                    return
                end
                ...
                if ok_lsp and lspconfig and lspconfig[srv] and type(lspconfig[srv].setup) == "function" then
                    ...
                    local avail = {}
                    if ok_lsp and type(lspconfig) == "table" then
                        for k,_ in pairs(lspconfig) do table.insert(avail, k) end
                    end
                    vim.notify(("LSP '%s' non trovato; available keys: %s")
                        :format(srv, table.concat(avail, ", ")), vim.log.levels.WARN)
                end
            end,
        })
